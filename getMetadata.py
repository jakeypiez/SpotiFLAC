...
def get_json_from_api(api_url, access_token, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if req.status_code == 401:
                print(f"WARNING: Access token expired, attempt {attempt + 1}/{max_retries}")
                if attempt < max_retries - 1:
                    sleep(2)
                    continue
                else:
                    raise SpotifyWebsiteParserException("ERROR: Access token expired after retries")

            if req.status_code != 200:
                if attempt < max_retries - 1:
                    print(f"WARNING: API request failed with {req.status_code}, retrying in {attempt + 1} seconds...")
                    sleep(attempt + 1)
                    continue
                else:
                    raise SpotifyWebsiteParserException(f"ERROR: {api_url} gave us {req.status_code} after {max_retries} retries")
                    
            return req.json()
            
        except requests.exceptions.Timeout:
            if attempt < max_retries - 1:
                print(f"WARNING: Request timeout, retrying in {attempt + 1} seconds...")
                sleep(attempt + 1)
                continue
            else:
                raise SpotifyWebsiteParserException(f"ERROR: Request timeout after {max_retries} retries")
                
        except requests.exceptions.ConnectionError:
            if attempt < max_retries - 1:
                print(f"WARNING: Connection error, retrying in {attempt + 1} seconds...")
                sleep(attempt + 1)
                continue
            else:
                raise SpotifyWebsiteParserException(f"ERROR: Connection error after {max_retries} retries")
